...
**已知残留（MCP 双 cordis）**：profile 里 link: 到本仓工作区的插件（mcp-settings 等）在装配 runtime 下报 "no credentials service mounted"——插件自带独立 cordis 模块副本（其 node_modules 的 registry cordis ≠ runtime 树 cordis，不同 store 路径），Service 注册表按模块实例隔离。源码 `dsh web` 无此问题恰因两者 cordis 同路径（link 到 checkout）。修法方向同桥的 cordis 符号链接技巧：插件安装形态改为 git tag 独立分发时，其 cordis 解析对齐 runtime 树；或 mcp-settings 的 manager 不 extends Service 改用纯函数注册。
