...
- **何时打桌面 `v*`**：壳 / IPC / 打包 / runtime 解压 / 标题带更新 / desktop-owned 八包升版 → 必须 bump 仓根 `package.json` 并推 `v<semver>`。纯插件且不在 desktop-owned 清单（如 mcp-settings、provider-balance、branding）→ 只打 `<包名>-v<semver>`。壳没变不要发桌面版。同一天能合并的壳修复合成一版，避免同事连下两次约 450MB。
...
- 壳的 release 只携带其运行面直接依赖的桌面自有插件，不按 `plugin/*` 无差别收包。当前集合是 bridge 0.2.0-rc.9（`bridge.tar.gz` → `~/.dsh-desktop/bridge/`；0.2.0-rc.3 起标题栏融合 CSS 首条规则 `html,body{overflow:hidden;}` 锁死根滚动；0.2.0-rc.4 起安装确认框展示 CHANGELOG 抽取的更新说明；0.2.0-rc.5 起 IPC 载体为 `window.__DSH_DESKTOP_IPC__`，兼容归档 Tauri 的 `__TAURI_INTERNALS__`；0.2.0-rc.6 起系统通知带 sessionId，点击回跳 `sessions.open`；0.2.0-rc.7 起新建/打开会话的 agent 附着脉冲不出 turn-done；0.2.0-rc.8 起更新入口改为点击弹窗下载——进度条 + 取消下载（新增 `dsh_desktop_cancel_update` IPC）+ ready 后「稍后 / 重启以更新」，不再发现即后台自动下载，决策见 `docs/notes/2026-08-28-update-download-dialog.md`；0.2.0-rc.9 起弹窗图标块为静态 app logo（`update-logo.tsx`），旋转只留在带内按钮；同版壳侧修复更新退出序列 veto 并加 slim 运行时预置，决策见 `docs/notes/2026-08-31-update-restart-fix.md`）、层次压缩兼容 Provider 0.1.3（`compaction-hierarchical.tar.gz` → `~/.dsh-desktop/plugins/dsh-compaction-hierarchical/`）、Web Search 开关 0.1.4（`web-search-toggle.tar.gz` → `~/.dsh-desktop/plugins/dsh-web-search-toggle/`）、模型图片输入 0.1.1（`model-image-input.tar.gz` → `~/.dsh-desktop/plugins/dsh-model-image-input/`；无需 runtime peer 链接——client bundle 零 `@deepseek-ai/*` 值导入、host 半空 apply）与运行中发送按钮 0.1.1（`send-while-running.tar.gz` → `~/.dsh-desktop/plugins/dsh-send-while-running/`；同为零值导入的 browser-only 形态、无 peer 链接；0.1.1 起 Stop 按钮任何状态染主题柔和红，与蓝 Send 视觉区分）、模型档位编辑器 0.1.1（`model-efforts-editor.tar.gz` → `~/.dsh-desktop/plugins/dsh-model-efforts-editor/`；同为零值导入的 browser-only 形态、无需 peer 链接——dsh-model-image-input 同类纯 DOM 注入，在 stock 模型设置卡每条 pi-ai 模型行内提供推理档位内联编辑：三态跟随默认/不推理/自定义档位+线上值 + Z.ai compat 勾选；决策见 `docs/notes/2026-08-27-dsh-model-efforts-editor.md`）与问题刻度尺 0.6.0（`question-rail.tar.gz` → `~/.dsh-desktop/plugins/dsh-question-rail/`；同为零值导入的 browser-only 形态、无需 peer 链接——占用加性槽 `conversation.input.dock`，≥6 条用户提问时对话左缘渲染等距刻度尺，悬停展开 Menu 观感列表、点击平滑跳转高亮，锚点随 sticky 输入区参与列布局运动；0.5.0 起刻度与展开条目共用 32px 槽位网格（一个刻度对应一个展开项、展开原地加宽零跳动，去掉面板标题栏），面板滑到顶部仍按需补页（挂载路径零分页，正文懒加载不受影响）；决策见 `docs/notes/2026-08-31-dsh-question-rail.md`）与 Thread 交接 0.2.0-rc.4（`thread.tar.gz` → `~/.dsh-desktop/plugins/dsh-thread/`；显式跨会话 Handoff 双面插件：bundle 层全局挂载 `thread_handoff` 工具 + 通用设置总开关（settings 命名空间 `dsh-thread`，默认开）+ 看板/侧栏分组/卡片 UI；host 半按 package.json `dependencies`/`peerDependencies` 链到 assembled runtime（缺 dependency 即 fail loud）防模块身份分裂；0.2.0-rc.3 起 host bundle 内联 zod（zod 归 devDeps + tsdown `deps.onlyBundle`，web-search-toggle 同款）——桌面解包姿态无 node_modules、仅有该链接表，裸 `zod` import 会让 loader entry 全挂、boot 即死（rc.23/rc.24 无法启动的根因），且 typert codec 走 duck check、内联副本合法；0.2.0-rc.4 起对齐 0.1.2 删除的 `settingsNamespace()`——`register` 直接收命名空间字符串（wst 0.1.4 同款；rc.25/rc.26 仍无法启动的第二层根因，rc.27 才是 Thread 进包后首个可启动版，rc.23–rc.26 用户均需手动下载）；决策见 `docs/notes/2026-08-21-session-thread.md`、`docs/notes/2026-09-02-thread-desktop-owned.md`、`docs/notes/2026-09-02-thread-zod-inline.md` 与 `docs/notes/2026-09-02-packaged-profile-smoke.md`）：prepare 对八包分别 typecheck/test/build、记录 tarball hash，并额外把 Web Search / model-image-input / send-while-running / model-efforts-editor / question-rail / thread 的版本写进 revision manifest；壳首启原子解压并把八包作为同一事务幂等 `plugin add` 到 web Profile。层次压缩的根 patch 刻意为空，安装只保证用户 preset 可在隔离 `compaction` realm 解析该 Provider，不替用户改 shipped/default preset；Web Search 插件持久化 home patch，并以 Host assembly/guard 对所有 Agent Preset 执行开关策略。随 `.app` 发货的插件在自己的 `package.json` 标 `dsh.desktop.ship: true`（可选 `tarball` / `dest` / `env` / `pin`）；prepare、壳解压、`plugin add`、CI 检查与 packaged `--dump-config` 冒烟都扫这个字段。新插件不要改 workflow。独立插件 tag 不能替代 desktop Release。
...
| `dsh_desktop_download_update` | — | 单飞执行重新检查→逐 chunk 下载→签名校验；校验后的包仅暂存在当前壳进程内，完成后推进到 `ready`，不安装、不重启。被 `dsh_desktop_cancel_update` 取消时正常返回（不报错），状态回到 `available`。**mac 增补（2026-08-31 起）**：zip 校验后、宣告 `ready` 前追加 slim 运行时预置——从下载的 zip 读 `runtime-revision.json`，若 `runtime/<sha>/.ok` 未命中则**异步**预拉 `runtime-<sha>-<platform>-<arch>.tar.gz` 到 `runtime-tarballs/` 缓存并校 sha256（相位保持 `downloading`、按 `.part` 字节上报进度、total 未知），预置失败则整体 `failed`（留在旧版、可重试，绝不带着缺失运行时重启）；取消会同时 abort 预拉的 curl。Windows NSIS 自带 runtime tar，不预置。 |
...
| `dsh_desktop_switch_surface` | — | 运行面切换（2026-08-28 起，见「功能面」M2 运行面切换条）：**不带渲染端入参**——壳弹原生右键菜单（「切换运行面…」），菜单项触发壳侧全流程：目录选择器（默认开在 `$DSH_HOME/profiles`）→ 校验 → 确认 → 首次切换跑八包事务 → 重启 sidecar → 窗口重载。归档 Tauri 无此命令，插件 invoke 失败只记日志。 |
