...
| `dsh_desktop_version_info` | — | 返回编译进桌面包的 `{ desktopVersion, runtimeVersion, runtimeSha }`，供 About 页展示；不读取用户目录。 |
| `dsh_desktop_check_update` | — | 查询更新端点（M3 起）：有更新返回 `{ update: { version, notes } }`，无则 `{ update: null }`；同时推进共享更新状态；未配置/不可达时返回错误文案（软失败，后台指示器静默）。 |
| `dsh_desktop_update_status` | — | 返回进程级更新状态快照：`idle/checking/current/available/preparing/downloading/installing/restarting/failed`；下载态含 `downloaded` 累计字节与可选 `total`，供 About 与标题带入口轮询实时进度。 |
| `dsh_desktop_apply_update` | — | 单飞执行重新检查→下载（逐 chunk 更新共享进度）→签名校验→安装→重启；成功即进程替换，调用方不再收到返回；并发调用或失败返回错误文案。 |
...
4. **About + 更新入口** —— `settings.section` 恢复 `desktop-about` 页面（order 100），展示桌面版本、内置 Harness runtime tag/commit，并在打开时读取共享更新状态；可手动检查、查看 release notes、一键下载校验安装并自动重启，下载时展示累计字节、总大小与实时百分比。`shell.overlay` 同时保留 `desktop-update-indicator`（order 6）：挂载 3s 首查走共享单飞记忆化，之后每 2h 强制刷新；离线/无端点静默，发现新版才在窗口右上角亮出下载图标，更新中同步显示百分比。两处通过 `dsh_desktop_update_status` 读取 Rust 进程级状态，不各自猜测下载阶段；应用动作单飞，重复点击不会启动第二份下载。借鉴 Zed/GitHub Desktop 的共识模式：后台检查不打扰，安装仍由用户明确触发。
...
- UI 只经 `ctx.slots.register(...)` 组合；本插件只注册**已声明的加性槽**——`shell.overlay`（badge/拖拽条/带内开关/更新指示器）与 `settings.section`（About）——声明洞一律禁止。
