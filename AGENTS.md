...
4. **标题带更新入口** —— 仅经 `shell.overlay` 插件实现：挂载 3s 后首查，之后每 2h 强制刷新；离线、无端点或已是最新版时完全静默。macOS 发现新版后在左上角标题带的侧栏开关旁出现下载按钮（收起态的 `+` 新会话气泡仍在其右侧）；其他平台保留右上角 fallback。用户点击后按钮原位持续旋转，`dsh_desktop_update_status` 提供实时字节进度；签名校验完成进入 `ready` 并弹确认框，只有确认“安装并重启”才消费暂存包、安装和重启。检查、下载、安装各自单飞，下载不等于授权安装。
