...
**形态（用户定稿）**：刻度与展开条目共用 32px 槽位网格（`RAIL_SLOT_PX`）——
`railGeometry` 按刻度数 × 32px 定尺高（封顶 320），收起时第 i 个刻度与展开后
第 i 个问题严格同 Y：悬停展开 = 原地加宽显字，零布局跳动（「丝滑」的来源）。
面板标题栏（「我的问题（N）」）移除，panel inset padding 归零（4px 内边距会
让行 i 偏离刻度 i）。槽距由 (i+0.5)/N 等距改为固定 32px 槽距——刻度数少时
尺身自然变短。面板滑到顶部分页补载（0.4.0）保留；补页后条目仍按槽高流动，
底部对齐时最近 10 条与刻度严格 1:1。

## 0.5.1：抽屉揭示动画（双层常驻 + 交叉淡入淡出）

**问题**：0.5.0 的展开/收起动画不丝滑——刻度层与列表层瞬时切换（display
none ↔ 条件渲染），且宽度动画期间 300px 内容在仍狭窄的容器里重排挤压。

**修法（抽屉揭示）**：刻度层（16px）与列表层（固定 300px）**常驻都挂载**，
外窗只做 width 16↔300 动画（overflow hidden 揭示）——内容层从不重排，只是
被揭开；两层交叉淡入淡出（刻度 0.1s 出 / 列表 0.15s 延迟 0.05s 入）。
列表常驻附带收益：滚动位置跨悬停保持，首次展开才滚到底。刻度 Y 定位从
百分比+6px inset 改为与行完全相同的 `(i+0.5)×slotPx` 公式，1:1 对齐严格化。

## 0.6.0：滚动跟随绑定（scroll-spy）

**需求**：刻度尺与「当前停在哪个用户问题」绑定——当前问题的刻度常亮品牌色，
随滚动自动移动。

**实现**：`computeActiveKey`（纯函数）：滚动体内 user/steering 消息行里最后一条
顶边越过参考线（视口上 35% 处）的即当前问题；滚动监听 100ms 节流
（timer.throttle），与 120ms 几何轮询共用一个 effect、滚动体换绑时重挂。
`windowTicks`（纯函数）：刻度窗口默认最近 10 条，当前问题滑出窗口时窗口居中
滑动跟随——当前刻度永远在尺上。展开面板里对应条目同步高亮；首次展开定位到
当前问题（居中）而非底部。激活态样式：刻度品牌色加宽 2px，条目 interactive
hover 底色。
