...
在既有「检查 / 下载 / 确认安装」拆分之上，把**下载**从用户点击改成发现新版后的后台自动行为；**安装**仍必须显式确认，且确认框只在用户点击已下载图标后打开。后续视觉复核撤回了控件底部进度条：下载态保持 22px 原位旋转，百分比只保留在 tooltip，避免在紧凑标题带里堆叠额外图形。
...
2. `preparing` / `downloading`：22px 控件只显示原位旋转图标，不渲染进度条；有 `total` 时百分比仅进入 tooltip，`prefers-reduced-motion` 下停止旋转。
...
- 下载态保持 22px，只渲染 spinner，并明确不含 `progressbar` 或进度条 DOM。
