...
实现在 `src/darwin-dock-guard.ts`，sidecar `--import` 与 `node-shim` 由 `src/runtime.ts` 接线；sidecar **进程本身** 也经 `dsh-pgrp` spawn（`src/sidecar.ts` `planSidecarSpawn`），否则 `detached`/`setsid` 仍会让 Dock 闪一颗马上消失的图标（`docs/notes/2026-08-28-sidecar-dock-flash.md`）。clang / 加载失败则 warn、行为与改前相同。Windows / two-node 不走这条。
