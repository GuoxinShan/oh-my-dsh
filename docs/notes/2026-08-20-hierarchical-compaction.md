...
The summarizer delegates fitting inputs to `super.summarize()` unchanged. If the Provider still classifies that attempt as `CONTEXT_WINDOW_EXCEEDED`, the same uncommitted transaction falls back to bounded mapping; other failures propagate unchanged. Oversized inputs use sequential map calls over tool-balanced message chunks, followed when needed by bounded recursive reduction of structured partial checkpoints. The summary model's declared context window and `chunkInputRatio` set the initial complete input budget. The inherited one-shot output cap participates in the delegation decision; if it cannot fit, the hierarchy runs directly. Map/reduce output caps must fit outside the hierarchy input budget.

A Provider-confirmed overflow from a planned map or reduce call is feedback that the initial estimator or advertised capacity was insufficient. The hierarchy bisects only that failed span at a tool-balanced map-unit or partial-checkpoint boundary and retains successful siblings. Every work item carries stable inclusive coordinates in the original map-unit sequence, so prompts and partial frames remain truthful when the pending call count changes. Instruction reserves price the widest possible coordinate text (`totalUnits-totalUnits of totalUnits`), not the shortest range, so a span whose ordinals have more digits than the reserve assumed cannot overflow solely from prompt undercounting. A non-singleton split strictly reduces the number of atomic inputs in each child; a singleton overflow is terminal. A reduce split must still lower partial cardinality for the round, otherwise the hierarchy fails without starting an identical next round; this deliberately makes a Provider-rejected final two-partial merge terminal rather than attempting singleton rewrites, which have no provable size decrease. Local splits do not consume recursive reduce depth.

Model-switch timing is a harness property, not a plugin one: Web model selection becomes durable only when a request consumes it, so the first post-switch `agent/pre-step` still prices pressure against the previous model's capacity. The first oversized request under the new model therefore dispatches, and stock context-overflow recovery — which reads the by-then-durable header — routes into this plugin's adaptive splitting. Closing the one-request gap proactively requires exposing the assembled selection at pre-step time inside the harness.
...
A multi-call operation does not set `SummaryResult.llmStreamCall: true`, because that marker asserts exactly one call through `ctx.llm.stream()`. A hierarchy that needs one successful map call and no prior failed attempt does set the marker honestly. The result stores the final stage output and aggregates disjoint usage only when every call reports it and no failed LLM attempt preceded the result; a failed one-shot or adaptive parent call omits the aggregate rather than representing a partial total. Intermediate calls remain private compaction traffic and are not separately resumable in this version.
...
Focused tests cover tool-balanced chunk planning, weighted adaptive splits, stable source coordinates, successful-sibling retention, provider-confirmed atomic overflow, reduce backoff, no-progress rejection, structured summary validation, hierarchy config validation, and aggregate usage. Package verification runs typecheck, node:test, tsdown build, and repository diff checks. A live preset mount remains the acceptance test for the installed runtime plane.
