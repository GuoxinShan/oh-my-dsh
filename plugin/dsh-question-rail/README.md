...
- 侧边刻度恒为最近 10 条（0.3.0 起）；**悬停展开的面板（0.4.0 起）是完整列表：滑到列表顶部即按需补载
  更早的问题，直到全部历史就位（`shouldPanelPage`，用户滑动驱动、逐页拉取，正文懒加载完全不受影响
  ——挂载路径零分页，只有你滑动面板时才动历史；面板补页时滚动位置自动保持。
  点击刻度/条目仍是「点击即达」：目标行不在 DOM 就先补载到出现再平滑跳转高亮。
  （0.2.0 的「全量载入」与 0.3.0 的「挂载时填充」经评审回滚，见决策记录。）
