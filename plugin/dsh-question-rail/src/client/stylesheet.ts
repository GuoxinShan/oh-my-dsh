...
export function questionRailCss(): string {
  return [
    ...
    '  /* Drawer-reveal motion: the inner layers keep their own fixed widths and',
    '     cross-fade, only the window width animates (0.5.1). */',
    ...
    '  opacity: 0.75;',
    '  cursor: pointer;',
    '  /* Inline top centers the tick on its slot, exactly like its row. */',
    ...
    '}',
    '/* Scroll-spy (0.6.0): the tick of the question the reader is at stays lit,',
    '   slightly wider than the hover state so it reads at a glance. */',
    '.dsh-qr-tick-active {',
    '  background: var(--dsw-alias-brand-primary);',
    '  opacity: 1;',
    '  height: 4px;',
    ...
    '  /* Drawer content: fixed width inside the animating window — no reflow,',
    '     just reveal. Cross-fades with the tick layer. */',
    '  opacity: 0;',
    '  pointer-events: none;',
    '  transition: opacity 0.15s ease 0.05s;',
    '}',
    '.dsh-qr-rail-open .dsh-qr-list {',
    '  opacity: 1;',
    '  pointer-events: auto;',
    '}',
    '.dsh-qr-item {',
    ...
    '  gap: 8px;',
    '  width: 100%;',
    '  /* Height comes inline from the slot grid (0.5.0). */',
    '  min-height: 0;',
    '  padding: 0 10px;',
    '  border: none;',
    '  border-radius: 10px;',
